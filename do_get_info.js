...
function getTime() {
    $("#getTime_").html('');
    var worksheets = [
        'Sheet4', // defaults to first worksheet without id
        '4'
    ];
    ...
    worksheets.forEach(function (worksheet) {
        $.googleSheetToJSON('1XjLtjloEkXKTDAsDLanA6Ace9ujINXbw7dClFAn5yeA', worksheet)
            .done(function (rows) {
                var strText = "<table border=1>";
                var strText = "<table class='dtable'>";
                strText += "<tr> <th>Ngày bắt đầu</th> <th>Số tuần</th>";
                var count = 0;
                rows.forEach(function (row) {
                    count++;
                    strText += "<tr>";
                    Object.getOwnPropertyNames(row).forEach(function (name) {
                        var val = [].concat(row[name]).join(' / ');
                        strText += "<td>" + "<input style='border: none' type='text' readonly value='"+ val +"' />" + "</td>";
