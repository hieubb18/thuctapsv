...
function sinhVienGet() {
    ...
    var pass = $.trim($("input[name='txtPass']").val()).replace(/ /g, '');
        ...
        $.googleSheetToJSON('1nO2nV65Vi3dZWGlaIOXLEc-_JWEZK16XFbjQVH_3Q0U', 10)
            .done(function (rows) {
                var count = 0;
                rows.forEach(function (row) {
                    var strMaSV = row['masv'];
                    var strPass = row['svhash'],
                        strScrphhash = row['scrphhash'];
                    var hash = ITHUTECHMD5(pass),
                        strChuoiinput = "",
                        strChuoioutput = "";
                    strChuoiinput = ITHUTECHMD5(hash + ITHUTECHMD5(row['scrphhash']));
                    strChuoioutput = ITHUTECHMD5(strPass + ITHUTECHMD5(row['scrphhash'])); 
                    
                    if (strMaSV == masv && strChuoiinput == strChuoioutput) { 
                        count ++;
                ...
                if (count == 0) {
                    $("#InfoSV").html(strBoxError);
                } else {

                    $("#InfoSV").html(strBoxSucess);
...
formBaoCao.addEventListener('submit', e => {
    ...
    if(areaText.length <= 30)
    {
        alert('BẠN CẦN NHẬP NOI DUNG IT NHAT 30 KI TU');
        return false;
    }
